...
public class AddCustomerVisitController {
    ...
    void initialize() {
        ...
        // Filter only persons WITHOUT positions (customers)
        allCustomers = persons.stream()
                .filter(p -> p.getPosition() == null || p.getPosition().isEmpty())
                .toList();

        personComboBox.setItems(FXCollections.observableArrayList(allCustomers));
        ...
        backButton.setOnAction(event -> switchScene("/org/example/fxml/CustomerVisits.fxml"));
    }
    ...
    private void handleSave() {
        ...
        if (selectedPerson == null || date == null || start.isEmpty()) {
            statusLabel.setText("Vyplňte povinné polia (Zákazník, Dátum, Príchod).");
            statusLabel.setStyle("-fx-text-fill: orange;");
            return;
        }

        if (dbManager.addAttendanceRecord(selectedPerson.getId(), date.toString(), start, end.isEmpty() ? null : end)) {
            statusLabel.setText("Návšteva uložená.");
            statusLabel.setStyle("-fx-text-fill: green;");
            switchScene("/org/example/fxml/CustomerVisits.fxml");
        } else {
    ...
    private void filterCustomers(String query) {
        if (query == null || query.isEmpty()) {
            ...
            List<Person> filtered = allCustomers.stream()
